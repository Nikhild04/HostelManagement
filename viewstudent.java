...
public class viewstudent extends JFrame implements ActionListener {
    ...
    viewstudent() {
        ...
        try {
            conn c = new conn();
            ResultSet rs = c.s.executeQuery("select * from student2");
            while (rs.next()) {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == search) {
            String query = "select * from student2 where empid = '" + cemployeeId.getSelectedItem() + "'";
            try {
                conn c = new conn();
